...
Milestone 8 (Resume Playback & Watch Progress) is **specified, not yet implemented** — see [SPEC.md §5.9](SPEC.md) and Milestone 8 in §19. It adds resume-vs-restart prompts for movies/episodes, watched-progress bars and completed checkmarks on thumbnails, and a `watch_progress` SQLite table. A "Skip Intro" button for TV series is captured as exploration only (§14, Q5) — IPTV providers supply no intro markers, so only a limited hybrid approach is feasible.
