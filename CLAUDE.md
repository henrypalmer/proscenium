...
- **The render thread is non-negotiable.** A single thread that both pumps window messages and renders freezes the video during a modal drag-resize (Win32) and starves resize hit-testing. So the UI thread does *only* the window message pump; a separate thread owns the GL context and renders (`mpv/player.rs::render_thread` / `render_thread_mac`). Teardown is **ordered**: the render context is freed on the render thread **before** the player handle is destroyed — enforced in `MpvPlayer::drop` (joins the render thread first).
- **Windows:** `mpv/mod.rs::video_host` creates a `WS_POPUP` tool window with `CS_OWNDC`; `mpv/mod.rs::render_win` brings up a WGL/OpenGL context on it and the player runs `vo=libmpv`. `lib.rs` also disables the main window's DWM maximize/restore/fullscreen transition (`DWMWA_TRANSITIONS_FORCEDISABLED`) so the animating frame can't lag the instantly-`SetWindowPos`-resized host window.
