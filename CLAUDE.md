...
- **The render thread is non-negotiable.** A single thread that both pumps window messages and renders freezes the video during a modal drag-resize (Win32) and starves resize hit-testing. So the UI thread does *only* the window message pump; a separate thread owns the GL context and renders. Teardown is **ordered**: a render context is freed on the render thread **before** its player handle is destroyed — the player's `pre_terminate` hook (Windows + macOS) removes its compositor tile (freeing its render context) first.
- **One shared compositor (Milestone 37, Windows + macOS).** **`mpv/compositor.rs`** owns *one* GL context + render thread on the native host surface and composites **N** mpv render contexts (one per tile/player; single playback = N=1) into N viewports via per-tile FBOs + `glBlitFramebuffer` (the shared `compositor::GlFns`). Players run `vo=libmpv` and register/unregister with the compositor (`commands/playback.rs::ensure_compositor` / `spawn_compositor_tile`); **`MpvPlayer` owns no render thread on either platform** — its `pre_terminate` hook removes its tile before the handle is destroyed. The per-platform GL plumbing lives behind `HostSurface` in `mpv/mod.rs::{render_win,render_mac}` (`create`/`begin_frame`/`lock`/`present`/`destroy`). Tile rects come from the frontend grid (`MultiView`) as **fractions (0..1)** of the player area; the compositor resolves them against its own live drawable each frame (`compositor::Rect::to_px`) — robust to the WebView viewport differing from the host surface (notably macOS, where the CSS viewport ≠ content size in points). `rect: None` = fill-window (auto-tracks resize) — the single-player case.
- **Windows host:** `mpv/mod.rs::video_host` creates a `WS_POPUP` tool window with `CS_OWNDC` that the compositor's WGL context binds to; the player's state callback self-heals its z-order. `lib.rs` disables the main window's DWM maximize/restore/fullscreen transition (`DWMWA_TRANSITIONS_FORCEDISABLED`) so the animating frame can't lag the instantly-`SetWindowPos`-resized host window.
- **macOS host:** `mpv/mod.rs::render_mac::create_gl_host` (main thread) builds a borderless `NSWindow` + `NSOpenGLContext` (3.2 core — deprecated desktop GL still works; reports `4.1 Metal` on Apple Silicon) bound to the window's content view, then glues it behind the main window via `video_host::{glue,fit_to_parent}` (objc2 `msg_send!`) as a child ordered *below*. `commands/playback.rs::ensure_gl_host` creates it up front and hands `(context, view)` to the **compositor** (not a per-player thread — the old `render_thread_mac` / `force-window` / `find_video_window` demote hacks are gone). The compositor's `render_mac::HostSurface` makes the context current on the render thread, presents via `flushBuffer`, guards each frame with `CGLLockContext`, and dispatches `-update` to the main thread on resize. Transparency needs `app.macOSPrivateApi: true` (tauri.conf.json) + the `macos-private-api` cargo feature. The 47 bundled dylibs must have **no `@rpath/` LC_RPATH** (see RELEASE.md) or dyld refuses to load libmpv. *(Known minor follow-up: a faint 1px seam can show at the top of the controls scrim over the transparent window on some mixed-DPI / external-monitor setups.)*
