...
**Status:** ✅ **Complete** — both slices. Slice 1: source **ranking** (resolution → debrid-cached → seeders → remembered source) + candidate **dedup** + **remember-the-pick** per title. Slice 2: opt-in **availability badges** — a rate-limited `index_availability` pass (≤8 titles/call) resolves + caches source count + best quality so cards badge **4K/HD/✓**; off by default (then the catalog behaves exactly as M40/M41). Tests: `milestone42` (5). Browser-preview verified the ranked picker (⚡/seeders), the badge toggle, and graceful degradation.

**Acceptance Criteria:**
- [x] Cards can show an **availability badge** populated by a background, non-blocking pass; disabled, the catalog behaves as M40/M41. *(Slice 2: an opt-in `availabilityBadgesEnabled` setting (default off) gates a rate-limited `index_availability` pass — for the canonical cards in view it resolves sources (series probe S1:E1), caches `(source_count, best_quality)` in the `availability` table (TTL 6h, ≤8 titles/call), and `CanonicalGrid` badges each card **4K/HD/✓**. Off → no badges and no background work. Test: `availability_cache_roundtrips_and_is_kind_scoped`. Browser-preview verified 19/24 cards badged once enabled.)*
...
- [x] `cargo test --tests` and `npm run build` pass clean. *(All 21 backend test binaries green, incl. `milestone42` (5 — ranking, dedup, addon cached/seeders parse, source-pref + availability cache round-trips); `npm run build` — tsc + vite — clean.)*

### Milestone 43 — Canonical Search (Cinemeta in the Search Flow)

**Goal:** Make multi-source-/addon-streamable titles findable from **search**, not only Browse. Until now search hit only the local FTS5 index of provider VOD, so a title no IPTV provider carries (but a Stremio addon could resolve) never appeared in results — a real gap once the catalog went canonical (M40) and addons became resolvers (M41). M43 folds the **canonical (Cinemeta) catalog** into both search surfaces alongside the provider results.

**Scope (approach A — separate group, no dedup):**
- A `search_canonical` command runs Cinemeta's movie + series search concurrently through the existing Tier-2 cache; a failure in either kind degrades to empty so the local results still stand (Cinemeta unreachable ≠ broken search).
- Both the search **overlay** and the full **results screen** fire the canonical lookup in parallel with the local FTS5 search (progressive: instant local results, canonical folds in when the network returns) and render the hits in an **"All Sources"** group/section, filtered by the active content-type tab.
- Activating a canonical hit opens its **canonical detail + source picker** (the M40 cross-provider/addon flow) via the Movies/TV Shows pages.
- Search no longer hard-gates on having an IPTV provider — canonical search works for **addon-only / no-provider** users.

**Out of scope (deferred polish):** cross-source **dedup** between a provider VOD hit and its canonical twin — a title can appear in both the provider group and "All Sources". Tracked as the follow-up (merge/dedup the two by tmdb/imdb via `content_match`, the C option), to land after this first pass is verified in real use.

**Status:** ✅ **Complete** — backend `search_canonical` (concurrent movie+series, cache-shared with browse, empty-on-failure) + both search surfaces render an "All Sources" canonical group routed to the canonical detail/picker, ungated from providers. Tests: `milestone43` (3 — search URL encoding, empty-search omission, search-body parse). Browser-preview verified the canonical group alongside provider results in both the overlay and the results screen, and a canonical hit opening its detail + source picker.

**Acceptance Criteria:**
- [x] Search surfaces **canonical (Cinemeta) hits** alongside the local provider results in both the overlay and the full results screen, fetched progressively and degrading to empty (never erroring) when Cinemeta is unreachable. *(`search_canonical` runs movie+series concurrently through the shared Tier-2 cache; both surfaces fire it in parallel with the local FTS5 search under one stale-guard and render an "All Sources" group. Test: `search_body_parses_into_canonical_hits`. Preview: 4 "All Sources" cards beside the provider Movies group in both surfaces.)*
- [x] Activating a canonical hit opens its **canonical detail + source picker** (the M40 multi-source flow), so a search result reaches the same playback path as Browse. *(Routed via `openCanonical` → `/movies` or `/shows` nav state → `CanonicalDetail` mounted as a sibling overlay, mirroring the existing provider-detail handoff. Preview: clicking a canonical hit navigated to `/movies` and rendered `canonical-detail` + `resolve-sources`.)*
- [x] Search works with **no IPTV providers enabled** — the "enable a provider" gate no longer blocks canonical results (addon-only / canonical-only users can search). *(Both surfaces drop the `hasProviders` hard-gate; the local FTS5 search is skipped without providers but the canonical lookup always runs, consistent with M40 canonical browse working provider-free.)*
- [x] `cargo test --tests` and `npm run build` pass clean. *(All 22 backend test binaries green, incl. `milestone43` (3 — search URL encode, empty-search omission, search-body parse); `npm run build` — tsc + vite — clean.)*

### Milestone 44 — Cross-Source Search Dedup

**Goal:** Finish the M43 search work by removing its one rough edge: a title carried by an IPTV provider **and** present in the canonical catalog appeared **twice** in search — once in the provider Movies/Series group and once in "All Sources". M44 collapses that duplicate, keeping the canonical ("All Sources") entry (whose picker already lists that provider *plus* any addons) and hiding the provider twin.

**Scope (approach C — the deferred M43 follow-up):**
- A `dedup_search_hits` command decides, per provider hit, whether it's the same title as one of the canonical hits: an authoritative `content_match` imdb when one has been recorded (a dupe iff that id is among the canonical hits — and *not* a dupe when it isn't, so a confirmed identity overrides a name/year lookalike), else the resolver's name+year match (`title_similarity` ≥ 0.6 + year ±1). It returns the provider hit keys to hide.
- Both search surfaces compute the hide-set once **both** result sets are in and filter the provider groups; Live TV is never deduped, and any failure degrades to no-dedup (all hits shown). Progressive: provider results show immediately and the duplicates collapse when the canonical set arrives.
- The matching logic stays in Rust (shared with the M40 resolver, unit-tested); the frontend only builds the lightweight hit payloads (`lib/searchDedup.ts`) and applies the returned key set.

**Out of scope:** changing the shared `normalize_title` — so a *single-word* provider title with a bare trailing year ("Dune 2021" vs canonical "Dune") can fall just under the name threshold, while multi-word titles and bracketed years dedup cleanly. Left as a possible later recall tweak since `normalize_title` also drives resolver matching.

**Status:** ✅ **Complete** — `dedup_search_hits` (authoritative `content_match`, else name+year) + both surfaces hide provider duplicates of a canonical hit, keeping the canonical entry. Tests: `milestone44` (7). Verified in the browser preview: the wrapper→matcher returns the right hide keys (name match, year-guarded remake kept, unknown-year dupe, multi-word+bare-year dupe), the render-side filter removes exactly the hidden movie while leaving series/live, and a live search runs clean.

**Acceptance Criteria:**
- [x] A provider search hit that is the same title as a canonical ("All Sources") hit is **hidden from the provider group** in both the overlay and the full results screen; the canonical entry (with the cross-source picker) is what remains. Matching is authoritative `content_match` imdb when recorded, else name+year. *(`dedup_search_hits` → `resolver::is_search_dupe`; `applyHideKeys` filters the provider groups, preferring the canonical entry. Tests: `dedup_hides_recorded_and_name_year_dupes_only`, `name_and_year_match_is_a_dupe_without_a_recorded_match`, `a_recorded_match_absent_from_the_hits_overrides_a_name_lookalike`.)*
- [x] Dedup never hides a non-duplicate, never crosses kinds or touches Live TV, and **degrades to no-dedup** on any failure; it applies **progressively** (provider results show immediately, dupes collapse once the canonical set lands). *(Movies/series deduped independently; `computeSearchHideKeys` returns an empty set on failure or a missing side; the effect recomputes as either result set updates. Tests: `a_remake_year_apart_is_not_a_dupe`, `a_different_title_is_not_a_dupe`, `no_canonical_hits_means_nothing_is_a_dupe`. Preview: the render-side filter left series/live untouched.)*
- [x] `cargo test --tests` and `npm run build` pass clean. *(All 23 backend test binaries green, incl. `milestone44` (7 — the dedup decision across name+year, remake, and authoritative-imdb present/absent, plus the DB-backed command impl); `npm run build` — tsc + vite — clean.)*
