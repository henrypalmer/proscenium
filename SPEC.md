...
- [ ] **macOS:** the same render-API path works via **OpenGL** (render-API probe ✅ PASS 2026-06-26 — viability confirmed: `mpv_render_context_create(opengl)` → `0`, frames flow into our FBO, clean teardown on Apple M1 Pro / GL 4.1 Metal; Tier 2 onscreen NSOpenGL + dedicated render thread **owner-confirmed: drag-resize smooth**). The fallback (ship Windows-only, keep current macOS embedding) is **not** needed. *(Box stays unchecked until the production player implements it; the probe only de-risked it.)*
- [ ] The architecture is **ready for M37** (a second render context/viewport can be added without re-plumbing).
- [ ] `cargo test --tests` and `npm run build` pass clean.

**Risks:**
- ~~**macOS libmpv render-API support is untested**~~ — **resolved** by the 2026-06-26 macOS render-API probe (✅ PASS): the Homebrew `libmpv.2.dylib` supports the OpenGL render API and frames flow into a GL FBO we own. (There is no Metal render-API type in libmpv — the macOS path is OpenGL.)
- **macOS bundled-ffmpeg TLS:** the probe saw the Homebrew mpv's ffmpeg fail TLS on the real provider (`tls: Unknown error`) while public HTTPS HLS played fine. Likely a TLS-backend/cert config in the bundled build; resolve before M38 ships on macOS (it's a transport issue, independent of the render API).
